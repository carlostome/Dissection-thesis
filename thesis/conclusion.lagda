...
datatype. Our tail-recursive machine is proven both to be terminating and
...
require the utilization of any exotic flag or extraneous postulate.
...
regular universe. We have shown how the construction of the latter machine follows 
the same steps to those of the former. By doing so we tried to motivate the design
choices made in the generic setting; it is always hard to reason alone about
generic constructions because of their abstract nature. The most complicated
part of our development was to find the appropriate predicates and lemmas that
allowed us to show termination. Once the properties were correctly spelled, most
of them followed by induction over the arguments. The most complex proof of this
thesis is the proof of well-foundedness in the generic case. It does not follow
directly by induction and several extra definitions and lemmas were required to
prove it true. The correctness of the tail-recursive machine was immediately
obvious from the usage of type-indexed configurations and a type-indexed step
function.
...
a program (or proof) typechecks then it is \emph{mathematically} true.
Moreover, the type theory underlying Agda is constructive. A theorem can be
interpreted as function that transforms inputs into outputs. Within our work, we
can appreciate this in the fact that the proof of a relation being
\emph{well-founded} implies that we can construct the accessibility predicate
mechanically for any term in the domain of the relation. However, using Agda has
also its drawbacks. The experience using it as an interactive proof assistant is
far from being ideal: typechecking big modules is rather slow; most the theorems
and functions depend on the inductive structure of the input and a simple change
in the definition of the datatype results in massive changes to the codebase;
irrelevance in Agda is very primitive, for example we cannot have functions that
purely work on the irrelevant side, thus, its application is limited.
...
A long term goal of our work would be to export our development to a generic
universe capable of representing lambda calculus terms, and their evaluation as
a simple fold over the syntax. A tail-recursive machine equivalent to the
catamorphism in such universe would effectively be an abstract machine that
evaluates programs written directly in the lambda calculus.

Moreover, it would be worth to explore how to use a well-founded argument to
...
self-evident choice. Nonetheless, it is not straightforward to understand how
our development would fit for such function.
...
machine relies upon dependent types, thus, it is not unambiguous how suitable is
