...
module Thesis.Dissection.Load where
  ...
  private
    ...
    Prop-first-not-inj₂-⨁₁ : ∀ {X : Set} {R Q P : Reg} {alg : ⟦ P ⟧ X → X}
                            {f : Leaf (R ⨁ Q) X → Stack P X alg → UZipper P X alg ⊎ X}
                            → Prop-Not-inj₂ X (R ⨁ Q) P alg f → Prop-Not-inj₂ X R P alg (first-⨁₁ R Q P f)
    Prop-first-not-inj₂-⨁₁ pr (l , isl) s x feq = pr (inj₁ l , NonRec-⨁-inj₁ _ _ l isl) s x feq 

    Prop-first-not-inj₂-⨁₂ : ∀ {X : Set} {R Q P : Reg} {alg : ⟦ P ⟧ X → X}
                             {f : Leaf (R ⨁ Q) X → Stack P X alg → UZipper P X alg ⊎ X}
                             → Prop-Not-inj₂ X (R ⨁ Q) P alg f → Prop-Not-inj₂ X Q P alg (first-⨁₂ R Q P f)
    Prop-first-not-inj₂-⨁₂ pr (l , isl) s x feq = pr (inj₂ l , NonRec-⨁-inj₂ _ _ l isl) s x feq

    Prop-first-not-inj₂-⨂-2 : ∀ {X : Set} {R Q P : Reg} {alg : ⟦ P ⟧ X → X}
                              {l   : ⟦ R ⟧ X}
                              {isl : NonRec R l}
                              {f : Leaf (R ⨂ Q) X → Stack P X alg → UZipper P X alg ⊎ X}
                             → Prop-Not-inj₂ X (R ⨂ Q) P alg f → Prop-Not-inj₂ X Q P alg (first-⨂-2 R Q P f (l , isl))
    Prop-first-not-inj₂-⨂-2 pr (l , isl) s x feq = pr ((_ , l) , NonRec-⨂ _ _ _ l _ isl) s x feq


    mutual
      Prop-first-not-inj₂-⨂-1 : ∀ {X : Set} {R Q P : Reg} {alg : ⟦ P ⟧ X → X}
                               ...
                               → Prop-Not-inj₂ X (R ⨂ Q) P alg f → Prop-Not-inj₂ X R P alg (first-⨂-1 R Q P k f q)
      Prop-first-not-inj₂-⨂-1 {R = R} {Q} {P} {k = k} {f = f} pr (l , isl) s x feq
        = first-not-inj₂ Q P _ (k ∘ inj₂ ∘ (_,_ (coerce l isl))) (first-⨂-2 R Q P f (l , isl)) (Prop-first-not-inj₂-⨂-2 pr) s x  feq 
      ...
      first-not-inj₂ 0′ Q () k f pr s x eq
      first-not-inj₂ 1′ Q r k f pr s x eq with f (tt , NonRec-1′) s | inspect (f (tt , NonRec-1′)) s
      first-not-inj₂ 1′ Q r k f pr s x () | inj₁ x₁   | Is is
      first-not-inj₂ 1′ Q r k f pr s .y refl | inj₂ y | Is is = pr (tt , NonRec-1′) s y is
      first-not-inj₂ I Q (In r) k f pr s x eq = first-not-inj₂ Q Q r id (λ z z₁ → inj₁ (z , z₁)) (λ _ _ _ ()) (k tt ∷ s) x eq
      first-not-inj₂ (K A) Q r k f pr s x eq  with f (r , NonRec-K A r) s | inspect (f (r , NonRec-K A r)) s
      first-not-inj₂ (K A) Q r k f pr s x () | inj₁ x₁ | Is is
      first-not-inj₂ (K A) Q r k f pr s .y refl | inj₂ y | Is is = pr (r , NonRec-K A r) s y is
      first-not-inj₂ (R ⨁ Q) P (inj₁ r) k f pr s x eq = first-not-inj₂ R P r (k ∘ inj₁) (first-⨁₁ R Q P f) (Prop-first-not-inj₂-⨁₁ pr) s x eq                         
      first-not-inj₂ (R ⨁ Q) P (inj₂ q) k f pr s x eq = first-not-inj₂ Q P q (k ∘ inj₂) (first-⨁₂ R Q P f) (Prop-first-not-inj₂-⨁₂ pr) s x eq
      first-not-inj₂ (R ⨂ Q) P (r , q) k f pr s x eq  =
        first-not-inj₂ R P r (k ∘ inj₁ ∘ (_, q)) (first-⨂-1 R Q P k f q) (Prop-first-not-inj₂-⨂-1 {_} {R} {Q} {P} {_} {k} {q} {f} pr) s x eq
    ...
    load-not-inj₂ : ∀ {X : Set} (R : Reg) {alg : ⟦ R ⟧ X → X}
                   ...
                   → load R t s ≡ inj₂ x
                   → ⊥
    load-not-inj₂ R (In t) s x leq = first-not-inj₂ R R t id (λ x → inj₁ ∘ (_,_ x)) (λ _ _ _ ()) s x leq
  ...
  private
    ...
    first-lemma {X} R Q {alg} r k f ni s  t z x p with view {X} R Q {alg} r
    first-lemma {X} 0′ Q {alg} () k f ni s t z x (e , plug-e , plugm) | inj₁ (dr , mq , fst)
    first-lemma {X} 1′ Q {alg} r k f ni s t z x (e , plug-e , plugm) | inj₁ (() , mq , fst)
    first-lemma {X} I Q {alg} (In r) k f ni s t z x (e , plug-e , plugm) | inj₁ (tt , .(In r) , First-I)
      = first-lemma Q Q r id (λ l → inj₁ ∘ _,_ l) (λ _ _ _ ()) (k tt ∷ s) t z x (Prop-Init Q r (k tt ∷ s) t (Plug-∷ plug-e plugm))
    first-lemma {X} (K A) Q {alg} r k f ni s t z x (e , plug-e , plugm) | inj₁ (() , mq , fst)
    first-lemma {X} (R ⨁ Q) P {alg} .(inj₁ r) k f ni s t z x (e , plug-e , plugm)
      | inj₁ (.(inj₁ _) , mq , First-⨁-inj₁ {r = r} fst)
      with view {X} R P {alg} r | first-lemma R P {alg} r (k ∘ inj₁) (first-⨁₁ R Q P f) (Prop-first-not-inj₂-⨁₁ ni) s t z x
    first-lemma {X} (R ⨁ Q) P .(inj₁ r) k f ni s t z x (e , plug-e , plugm-e)
    ...
    first-lemma {X} (R ⨁ Q) P {alg} .(inj₂ q) k f ni s t z x (e , plug-e , plugm-e)
      | inj₁ (.(inj₂ dq) , mq , First-⨁-inj₂ {q = q} {dq} fst)
      with view {X} Q P {alg} q | first-lemma Q P {alg} q (k ∘ inj₂) (first-⨁₂ R Q P f) (Prop-first-not-inj₂-⨁₂ ni) s t z x
    first-lemma {X} (R ⨁ Q) P .(inj₂ q) k f ni s t z x (e , plug-e , plugm-e)
    ...
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z x (e , plug-e , plugm-e)
      | inj₁ (.(inj₁ (rx , q)) , mq , First-⨂₁ {rx = rx} fst)
      with view {X} R P {alg} r
      | first-lemma R P {alg} r (k ∘ inj₁ ∘ (_, q)) (first-⨂-1 R Q P k f q) (Prop-first-not-inj₂-⨂-1 {X} {R} {Q} {P} {alg} {k} {q} {f} ni) s t z x
    first-lemma {X} (R ⨂ Q)  P (r , q) k f ni s t z x (e , plug-e , plugm-e)
    ...
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z x (e , plug-e , plugm-e)
      ...
      | first-lemma Q P {alg} q (k ∘ inj₂ ∘ (_,_ (coerce r′ nr-r))) ((first-⨂-2 R Q P f (r′ , nr-r)))
                                (Prop-first-not-inj₂-⨂-2 {X} {R} {Q} {P} {alg} {r′} {nr-r} ni) s t z′ is′
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z x (e , plug-e , plugm-e)
      | inj₁ (.(inj₂ (coerce r nr , qx)) , mq , First-⨂₂ {qx = qx} nr fst) | inj₂ (r′ , nr-r , eq) | cont | inj₁ z′ | Is is′
      | inj₁ (dr′′ , mq′′ , fst′′) | cont′
      with First-unicity fst fst′′
    ...
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z x (e , plug-e , plugm-e)
      | inj₁ (.(inj₂ (coerce r nr , qx)) , mq , First-⨂₂ {qx = qx} nr fst) | inj₂ (r′ , nr-r , eq) | cont | inj₂ z′ | Is is′
        = ⊥-elim (first-not-inj₂ Q P q (k ∘ inj₂ ∘ (_,_ (coerce r′ nr-r))) (first-⨂-2 R Q P f (r′ , nr-r))
          (Prop-first-not-inj₂-⨂-2 {X} {R} {Q} {P} {alg} {r′} {nr-r} {f} ni) s z′ is′)
    first-lemma {X} 0′ Q () k f ni s t z x p
      | inj₂ (l , isl , plug)
    first-lemma {X} 1′ Q {alg} .tt k f ni s t z x p
      | inj₂ (.tt , NonRec-1′ , ≈-1′)
      with f (tt , NonRec-1′) s
    first-lemma {X} 1′ Q {alg} .tt k f ni s t .x refl p
      | inj₂ (.tt , NonRec-1′ , ≈-1′) | inj₁ x = p
    first-lemma {X} I Q {alg} r k f ni s t z x p | inj₂ (_ , () , eq)
    first-lemma {X} (K A) Q {alg} .l k f ni s t z x p
      | inj₂ (l , NonRec-K .A .l , ≈-K)
      with f (l , NonRec-K A l) s
    first-lemma {X} (K A) Q {alg} .l k f ni s t .x refl p
      | inj₂ (l , NonRec-K .A .l , ≈-K) | inj₁ x = p
    first-lemma {X} (R ⨁ Q) P {alg} .(inj₁ x) k f ni s t z e p | inj₂ (.(inj₁ r) , NonRec-⨁-inj₁ .R .Q r isl , ≈-⨁₁ {x = x} eq)
      with view {X} R P {alg} x | first-lemma R P x (k ∘ inj₁) (first-⨁₁ R Q P f) (Prop-first-not-inj₂-⨁₁ ni) s t z e
    first-lemma {X} (R ⨁ Q) P .(inj₁ x) k f ni s t z e p | inj₂ (.(inj₁ r) , NonRec-⨁-inj₁ .R .Q r isl , ≈-⨁₁ {x = x} eq)
      | inj₁ (dr , mq , fst) | _ = ⊥-elim (First-NonRec fst (≈-NonRec r isl x (≈-sym eq)))
    first-lemma {X} (R ⨁ Q) P .(inj₁ x) k f ni s t z e p | inj₂ (.(inj₁ r) , NonRec-⨁-inj₁ .R .Q r isl , ≈-⨁₁ {x = x} eq)
      | inj₂ (l , isl′ , eq-l) | cont
      with ≈-to-≡ (≈-trans (≈-sym eq-l) eq)
    first-lemma {X} (R ⨁ Q) P .(inj₁ x) k f ni s t z e p | inj₂ (.(inj₁ l) , NonRec-⨁-inj₁ .R .Q .l isl , ≈-⨁₁ {x = x} eq)
      | inj₂ (l , isl′ , eq-l) | cont | refl
      with NonRec-proof-irrelevance isl isl′
    first-lemma {X} (R ⨁ Q) P .(inj₁ x) k f ni s t z e p | inj₂ (.(inj₁ l) , NonRec-⨁-inj₁ .R .Q .l .isl′ , ≈-⨁₁ {x = x} eq)
      | inj₂ (l , isl′ , eq-l) | cont | refl | refl = cont p
    first-lemma {X} (R ⨁ Q) P {alg} .(inj₂ x) k f ni s t z e p | inj₂ (.(inj₂ q) , NonRec-⨁-inj₂ .R .Q q isl , ≈-⨁₂ {x = x} eq)
      with view {X} Q P {alg} x | first-lemma Q P x (k ∘ inj₂) (first-⨁₂ R Q P f) (Prop-first-not-inj₂-⨁₂ ni) s t z e
    first-lemma {X} (R ⨁ Q) P .(inj₂ x) k f ni s t z e p | inj₂ (.(inj₂ q) , NonRec-⨁-inj₂ .R .Q q isl , ≈-⨁₂ {x = x} eq)
    ...
    first-lemma {X} (R ⨁ Q) P .(inj₂ x) k f ni s t z e p | inj₂ (.(inj₂ l) , NonRec-⨁-inj₂ .R .Q .l .isl′ , ≈-⨁₂ {x = x} eq)
      | inj₂ (l , isl′ , eq-l) | cont | refl | refl = cont p
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z e p | inj₂ ((r′ , q′) , NonRec-⨂ .R .Q _ _ isl₁ isl₂ , ≈-⨂ eq₁ eq₂)
      with view {X} R P {alg} r
      | first-lemma R P r (k  ∘ inj₁ ∘ (_, q)) (first-⨂-1 R Q P k f q) (Prop-first-not-inj₂-⨂-1 {X} {R} {Q} {P} {alg} {k} {q} {f} ni) s t z e
    first-lemma {X} (R ⨂ Q) P (r , q) k f ni s t z e p | inj₂ ((r′ , q′) , NonRec-⨂ .R .Q _ _ isl₁ isl₂ , ≈-⨂ eq₁ eq₂)
      | inj₁ (dr , mq , plug) | _
      = ⊥-elim (First-NonRec plug (≈-NonRec r′ isl₁ r (≈-sym eq₁)))
    first-lemma {X} (R ⨂ Q) P (r , q) k f ni s t z e p | inj₂ ((r′ , q′) , NonRec-⨂ .R .Q _ _ isl₁ isl₂ , ≈-⨂ eq₁ eq₂)
      | inj₂ (l , isl , eq) | cont
      with ≈-to-≡ (≈-trans (≈-sym eq) eq₁)
    first-lemma {X} (R ⨂ Q) P (r , q) k f ni s t z e p | inj₂ ((.l , q′) , NonRec-⨂ .R .Q _ _ isl₁ isl₂ , ≈-⨂ eq₁ eq₂)
    ...
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z e p | inj₂ ((.l , q′) , NonRec-⨂ .R .Q _ _ .isl isl₂ , ≈-⨂ eq₁ eq₂)
      | inj₂ (l , isl , eq) | cont | refl | refl | inj₁ z′ | Is f-eq
      with view {X} Q P {alg} q
      | first-lemma Q P q (k ∘ inj₂ ∘ (_,_ (coerce l isl))) ((first-⨂-2 R Q P f (l , isl)))
                          (Prop-first-not-inj₂-⨂-2 ni) s t z′ f-eq
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z e p | inj₂ ((.l , q′) , NonRec-⨂ .R .Q _ _ .isl isl₂ , ≈-⨂ eq₁ eq₂)
    ...
    first-lemma {X} (R ⨂ Q) P {alg} (r , q) k f ni s t z e p | inj₂ ((.l , q′) , NonRec-⨂ .R .Q _ _ .isl isl₂ , ≈-⨂ eq₁ eq₂)
      ...
      = ⊥-elim (Prop-first-not-inj₂-⨂-1 {X} {R} {Q} {P} {alg} {k} {q} {f} ni (l , isl) s y is)
  ...
  load-preserves R (In x) s z leq t p = first-lemma R R x id (λ x → inj₁ ∘ (_,_ x)) (λ _ _ _ ()) s t z leq (Prop-Init R x s t p)
