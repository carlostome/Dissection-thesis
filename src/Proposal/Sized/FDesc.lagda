...
module Proposal.Sized.FDesc where
...
%<*FDesc>
\begin{code}
  data FDesc : {_ : Size} → Set₁ where
    I₁    : {i : Size}                  → FDesc {↑ i}
    K₁    : {i : Size}     → (A : Set)  → FDesc {↑ i}
    _+₁_  : ∀ {i : Size} {k j : Size< i} (P Q : FDesc) → FDesc {i}
    _×₁_  : ∀ {i : Size} {k j : Size< i} (P Q : FDesc)  → FDesc {i}
\end{code}
%</FDesc>
...
%<*fmap>
\begin{code}
  fmap : ∀ {i : Size} {A B : {_ : Size} → Set} (F : FDesc) → (A → B) → ⟦ F ⟧₁ A {i} → ⟦ F ⟧₁ B {i}
  fmap I₁       f x         = f x
  fmap (K₁ A)   f x         = x
  fmap {i} (P +₁ Q) f (inj₁ x)  = inj₁ (fmap {i} {_} P f x)
