...
module Proposal.TotallyFree.TailRec where
  ...
  module Term {I O : Set} (_<_ : Rel I) (wf : WF.WellFounded _<_)  where
    ...
    tail-rec-work : (inp : I) (f : I ⟶ O) → Terminates f → WF.Acc _<_ inp → O
    tail-rec-work inp f term wf with term inp
    ... | Ti with f inp
    tail-rec-work inp f term wf | tt | return x = x
    tail-rec-work inp f term (WF.acc ac) | x<i | call⟨ x ⟩ = tail-rec-work x f term (ac x x<i)
  ...
  module Examples where
    ...
    module Length {A : Set} where
      ...
      open Inverse-image-Well-founded {ℕ × List A} (_⊰L_) proj₂ public
