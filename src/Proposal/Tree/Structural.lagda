\begin{code}
module Proposal.Tree.Structural where
...
\begin{code}
  ...
  mutual
    ...
    accL t t₁ (y , s) s₁ x (acc rs) .(_ , Right _ _) (lt .(Node t s₁) .(_ , Right _ _) .(y , Left t₁ s) (≣-Node eq₁ eq₃) eq₂ (<S-Right-Left x₂ x₃))
      = acc (accR s₁ _ (_ , _) t eq₁ (<S-WF s₁ (_ , _)))
    accL t t₁ (y , s) s₁ x (acc rs) .(Node _ t₁ , Top) (lt .(Node t s₁) .(Node _ t₁ , Top) .(y , Left t₁ s) (≣-Node eq₁ eq₃) eq₂ (<S-Left-Base x₂))
      = acc (accH t s₁ _ t₁ eq₁ eq₃)

    accH : ∀ s₁ s₂ t₁ t₂ → t₁ ≣ s₁ → t₂ ≣ s₂ → WfRec ([_]ₛ_<_ (Node s₁ s₂)) (Acc ([_]ₛ_<_ (Node s₁ s₂))) (Node t₁ t₂ , Top)
    accH s₁ s₂ t₁ t₂ x x₁ .(_ , Right t₁ _) (lt .(Node s₁ s₂) .(_ , Right t₁ _) .(Node t₁ t₂ , Top) (≣-Node eq₁ eq₃) eq₂ (<S-Right-Base x₂))
      = acc (accR s₂ t₁ _ s₁ eq₁ (<S-WF s₂ (_ , _)))

    <S-WF : ∀ t → Well-founded ([ t ]ₛ_<_)
    <S-WF t x = acc (aux t x)
      where aux : ∀ t x → WfRec ([_]ₛ_<_ t) (Acc ([_]ₛ_<_ t)) x
            aux .(Node _ _) (x , Left t₁ s) .(t₂ , Left t₁ s₂) (lt .(Node _ _) .(t₂ , Left t₁ s₂) .(x , Left t₁ s) (≣-Node eq₁ eq₃) eq₂ (<S-Left-Step {t₁ = t₂} {s₁ = s₂} p))
              = acc (accL _ t₁ (t₂ , s₂) _ eq₃ (<S-WF _ (t₂ , s₂)))
            aux .(Node _ _) (x , Left t₁ s) .(_ , Right _ _) (lt .(Node _ _) .(_ , Right _ _) .(x , Left t₁ s) (≣-Node eq₁ eq₃) eq₂ (<S-Right-Left x₂ x₃))
              = acc (accR _ _ _ _ eq₁ (<S-WF _ (_ , _)))
            aux .(Node _ _) (x , Left t₁ s) .(Node _ t₁ , Top) (lt .(Node _ _) .(Node _ t₁ , Top) .(x , Left t₁ s) (≣-Node eq₁ eq₃) eq₂ (<S-Left-Base x₂))
              = acc (accH _ _ _ t₁ (eq₁) (eq₃))
            aux .(Node _ _) (x , Right t₁ s) .(t₂ , Right t₁ s₁) (lt .(Node _ _) .(t₂ , Right t₁ s₁) .(x , Right t₁ s) (≣-Node eq₁ eq₄) (≣-Node eq₂ eq₃)
              (<S-Right-Step {t₁ = t₂} {s₁ = s₁} p)) = acc (accR _ t₁ (t₂ , s₁) _ eq₂ (<S-WF _ (t₂ , s₁)))
            aux .(Node _ _) (.(Node _ _) , Top) .(_ , Right _ _) (lt .(Node _ _) .(_ , Right _ _) .(Node _ _ , Top) (≣-Node eq₁ eq₃) eq₂ (<S-Right-Base x₁))
              = acc (accR _ _ _ _ eq₁ (<S-WF _ _))

  to-left : Tree → Stack → Tree × Stack
  to-left (Leaf n)  s          = (Leaf n) , s
  to-left (Node t₁ t₂)  s = to-left t₁ (Left t₂ s)

  to-left-preserves-⊳ : ∀ t s t′ s′ → to-left t s ≡ (t′ , s′) → t ⊳ s ≣ t′ ⊳ s′
  to-left-preserves-⊳ (Leaf n) s .(Leaf n) .s refl    = ≣-reflexive
  ...
  to-up-right-preserves-⊳ t (Left t₁ s) .(Node t t₁) .s refl = ≣-reflexive
  to-up-right-preserves-⊳ t (Right t₁ s) t′ s′ x = to-up-right-preserves-⊳ (Node t₁ t) s t′ s′ x
  to-up-right-preserves-⊳ t Top t′ s′ ()

  right : (ℕ → ℕ) → Tree × Stack → Maybe (Tree × Stack)
  right _ (t , Left t₁ s)          = just (Node t t₁  , s)
  right f (Leaf n , Right t₁ s)  = to-up-right (Node t₁ (Leaf (f n))) s
  right _ (Node t₁ t₂ , Right t s) = just (t₂ , Right t₁ (Right t s))
  right _ (Leaf n , Top)              = nothing
  right _ (Node t₁ t₂ , Top) = just (to-left t₂ (Right t₁ Top))

  just-injective : ∀ {l} {A : Set l} {x y : A} → (Maybe A ∋ just x) ≡ just y → x ≡ y
  just-injective {l} {A} {x} refl = refl

  right-preserves-⊳ : ∀ f z z′ → right f z ≡ just z′  → plug-⊳ z ≣ plug-⊳ z′
  right-preserves-⊳ f (x , Left t s) .(Node x t , s) refl = ≣-reflexive
  right-preserves-⊳ f ((Leaf n) , Right t s) (y , s′) eq with to-up-right-preserves-⊳ (Node t (Leaf (f n ))) s y s′ eq
  ... | z = {!!}
  right-preserves-⊳ f (Node x x₁ , Right t s) (.x₁ , .(Right x (Right t s))) refl = {!!}
  right-preserves-⊳ f (Tip , Top) z′ ()
  right-preserves-⊳ f (Node x x₁ , Top) (y , s) eq = to-left-preserves-⊳ x₁ (Right x Top) y s (just-injective eq)

  ⊳-++ₛ-Right : ∀ {x} {t} s → x ⊳ (s ++ₛ Right t Top) ≣ Node t (x ⊳ s)
  ⊳-++ₛ-Right (Left t s)   = ⊳-++ₛ-Right s
  ⊳-++ₛ-Right (Right t s)  = ⊳-++ₛ-Right s
  ⊳-++ₛ-Right Top          = ≣-reflexive

  ⊳-++ₛ-Left : ∀ {x} {t} s → x ⊳ (s ++ₛ Left t Top) ≣ Node (x ⊳ s) t
  ⊳-++ₛ-Left (Left t s)   = ⊳-++ₛ-Left s
  ⊳-++ₛ-Left (Right t s)  = ⊳-++ₛ-Left s
  ⊳-++ₛ-Left Top          = ≣-reflexive

  ⊲-++ₛ-Right : ∀ {x} {t} s → x ⊲ (s ++ₛ Right t Top) ≣ Node t x ⊲ s
  ⊲-++ₛ-Right (Left t s)   = ≣-Node (⊲-++ₛ-Right s) ≣-reflexive
  ⊲-++ₛ-Right (Right t s)  = ≣-Node ≣-reflexive (⊲-++ₛ-Right s)
  ⊲-++ₛ-Right Top          = ≣-reflexive

  ⊲-++ₛ-Left : ∀ {x} {t} s → x ⊲ (s ++ₛ Left t Top) ≣ Node x t ⊲ s
  ⊲-++ₛ-Left (Left t s)   = ≣-Node (⊲-++ₛ-Left s) ≣-reflexive
  ⊲-++ₛ-Left (Right t s)  = ≣-Node ≣-reflexive (⊲-++ₛ-Left s)
  ⊲-++ₛ-Left Top          = ≣-reflexive
